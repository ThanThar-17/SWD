...
total = 0;
function calcTotal(numberArray) {
  for (x of numberArray) {
    total += x;
  }
  return total;
}
console.log(calcTotal(points));
...
function separateevenodd(numberArray) {
  ...
  for (x of numberArray) {
    if (x % 2 === 0) {
      evenArray[evenArrIndex++] = x;
    } else {
      oddArray[oddArrIndex++] = x;
    }
  }
  return {
    even: evenArray,
...
function sliceUpdate(array, startIndex, count) {
  const resultArray = [];
  let resultArrayIndex = 0;
  let endIndex = startIndex + count;
  for (x in array) {
    if (x >= startIndex && x < count) {
      resultArray[resultArrayIndex++] = array[x];
    }
  }
  return resultArray;
}
console.log(sliceUpdate(points, 2, 5));
...
const newPoints = CopyArray(points);
console.log(CopyArray(points));
console.log(newPoints === points); //heap address are not the same

function modifiedArray(array) {
  const result = [];
  let resultIndex = 0;
  for (x of array) {
    result[resultIndex++] = x + 10;
    // result[resultIndex++] = x % 2 === 0 ? "even" : "odd";
  }
  return result;
}
console.log(modifiedArray(points));

console.log("Original Product Table");
